...
const ModuleSidebar = ({ trans, session }) => {
  const t = useTranslations();
  const rawMenus = menusConfig?.sidebarNav?.modern || [];
  ...
  const filterMenus = (menus, session) => {
    ...
    const filterMenuItems = (items) => {
      return items
        .map((item) => {
          ...
          // If permissions array is not present or is empty, allow the item
          const permissionsExist =
            item.permissions && item.permissions.length > 0;

          return !permissionsExist || hasPermission(item.permissions)
            ? item
            : null;
        })
          ...
          <h2 className="text-lg  bg-transparent   z-50   font-semibold  flex gap-4 items-center   text-default-700 sticky top-0 py-4  px-4   capitalize ">
            <span className=" block ">
              {menus[activeIndex]?.title
                ? `${t(`sideLinks.${menus[activeIndex]?.title}`)}`
                : " "}
            </span>
            {!isDesktop && (
              <Button
