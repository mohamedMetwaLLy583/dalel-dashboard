  ...
  providers: [
    ...
    Credentials({
      ...
      async authorize(credentials) {
        ...
        const backendBaseUrl = process.env.BACKEND_URL || process.env.NEXT_PUBLIC_BACKEND_BASE_URL;
        const res = await fetch(
          `${backendBaseUrl}/api/dashboard/login`,
          {
            method: 'POST',
            headers: {
    ...
    async jwt({ token, user }) {
      ...
      if (token.accessToken) {
        ...
        const validationResponse = await fetch(
          `${backendBaseUrl}/api/dashboard/check-token`,
          {
            method: 'POST',
            headers: {
