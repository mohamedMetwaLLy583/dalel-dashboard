...
export default function EditMarketingStepsForm() {
  const {
    register,
    formState: { errors },
    handleSubmit,
  ...
  const { EditData, loading, currentData, getCurrentData } =
    useMarketingStepsHook();

  const [imagePreview, setImagePreview] = useState(null);

  const img = watch("icon");
  ...
  useEffect(() => {
    ...
    setImagePreview(currentData?.icon);
  }, [currentData]);
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="grid grid-cols- gap-6">
        ...
        <div className="col-span-2  flex flex-col gap-2">
          ...
          >
            {t("MarketingSteps.StoreMarketingSteps.titleEn")}{" "}
            {errors["title_en"]?.type === "required" && "*"}
          </Label>
          <Input
            type="text"
            placeholder={t(
              "MarketingSteps.StoreMarketingSteps.placeholders.titleEn"
            )}
            id="title_en"
          ...
          {errors["title_en"]?.type === "minLength" && (
            <Label htmlFor="title_en" className="text-red-600 mt-2">
              {t("MarketingSteps.StoreMarketingSteps.formErrors.titleMinL")}
            </Label>
          )}
        </div>
        <div className="col-span-2  flex flex-col gap-2">
          ...
          >
            {t("MarketingSteps.StoreMarketingSteps.titleAr")}{" "}
            {errors["title_ar"]?.type === "required" && "*"}
          </Label>
          <Input
            type="text"
            placeholder={t(
              "MarketingSteps.StoreMarketingSteps.placeholders.titleAr"
            )}
            id="title_ar"
        ...
        <div className="col-span-2  flex flex-col gap-2">
          ...
          >
            {t("MarketingSteps.StoreMarketingSteps.descEn")}{" "}
            {errors["description_en"]?.type === "required" && "*"}
          </Label>
          <Textarea
            placeholder={t(
              "MarketingSteps.StoreMarketingSteps.placeholders.descEn"
            )}
            id="description_en"
            rows="3"
          ...
          {errors["description_en"]?.type === "minLength" && (
            <Label htmlFor="description_en" className="text-red-600 mt-2">
              {t("MarketingSteps.StoreMarketingSteps.formErrors.descMinL")}
            </Label>
          )}
        </div>
        <div className="col-span-2  flex flex-col gap-2">
          ...
          >
            {t("MarketingSteps.StoreMarketingSteps.descAr")}{" "}
            {errors["description_ar"]?.type === "required" && "*"}
          </Label>

          <Textarea
            placeholder={t(
              "MarketingSteps.StoreMarketingSteps.placeholders.descAr"
            )}
            id="description_ar"
            rows="3"
        ...
        {/* Main Image */}
        <div className="col-span-2  flex flex-col gap-2">
          <Label>{t("MarketingSteps.StoreMarketingSteps.image")} </Label>
          <Input type="file" variant="flat" {...register("icon")} />
        </div>
        {imagePreview && (
          <div className="col-span-2  flex flex-col gap-2">
            <img
